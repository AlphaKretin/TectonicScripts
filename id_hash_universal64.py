# Universal Dictionary VLE with 64 tokens
# Good balance: 40.99 bits avg, 92 bits max
#
# Encoding scheme:
...
def build_dictionary(all_ids, max_tokens=64, min_freq=5):
    ...
    def token_value(token, freq):
        ...
        savings_per_use = individual_cost - 10  # 10 bits for dictionary token
        if savings_per_use < 2:
            return 0
...
def test_ids(ids):
    ...
    dict_tokens = build_dictionary(ids)
...
if __name__ == "__main__":
    ...
    avg, max_b, tokens = test_ids(all_ids)

    print(f"Universal Dictionary VLE (64 tokens)")
    print("=" * 50)
    print(f"Average: {avg:.2f} bits")
    print(f"Max: {max_b} bits")
